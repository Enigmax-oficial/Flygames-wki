...
replaceInFile('src/components/AdminPanel.tsx', content => {
  content = content.replace(/\/\/ Validate authorized email\n\s*const authorizedEmails = \[.*?\];\n\s*const isAuthorized = userEmail && authorizedEmails\.includes\(userEmail\.toLowerCase\(\)\.trim\(\)\);\n/g, '');
...
replaceInFile('src/components/Header.tsx', content => {
  content = content.replace(/\/\/ Validate authorized email\n\s*const authorizedEmails = \[.*?\];\n\s*const isAdmin = userEmail && authorizedEmails\.includes\(userEmail\.toLowerCase\(\)\.trim\(\)\);\n/g, 'const isAdmin = true; // Authentication 2.0 open to challenge\n');
  return content;
});
...
replaceInFile('server.ts', content => {
  content = content.replace(/\s*const authorizedEmails = \[.*?\];\n\s*if \(!email \|\| !authorizedEmails\.includes\(email\.toLowerCase\(\)\.trim\(\)\)\) \{[\s\S]*?\}\n/g, '\n');
