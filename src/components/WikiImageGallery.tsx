...
export const WikiImageGallery: React.FC<WikiImageGalleryProps> = ({ items, pageTitle }) => {
  ...
  return (
    <div className="bg-[#0f172a]/95 border border-sky-500/20 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-md relative font-sans my-4">
      {/* Gallery Header Bar */}
      <div className="bg-[#0b0f19] px-4 py-3 border-b border-[#1e293b] flex items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1.5 text-[#94a3b8] hover:text-white bg-[#1e293b]/60 hover:bg-[#1e293b] rounded transition-colors cursor-pointer"
      ...
      {/* Main Image Stage */}
      <div className="relative min-h-[300px] sm:min-h-[380px] bg-[#070a12] flex items-center justify-center p-4 overflow-hidden select-none">
        ...
        {!is3DModelActive && (
          <div className="relative z-10 flex flex-col items-center justify-center max-w-full p-4">
            <img
              src={currentItem.url}
              alt={currentItem.caption || pageTitle}
              className="max-h-[280px] sm:max-h-[340px] w-auto object-contain drop-shadow-[0_15px_30px_rgba(0,0,0,0.8)] transition-all duration-300 hover:scale-105"
            />
          </div>
        )}
      ...
      {isFullscreen && (
        ...
        >
          <div className="w-full max-w-5xl flex items-center justify-between text-white border-b border-[#334155] pb-4">
            <h3 className="font-bold text-lg text-sky-400 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-sky-400" />
              <span>{pageTitle} - Showcase Viewer</span>
            </h3>
            <button
              onClick={() => setIsFullscreen(false)}
              className="p-2 bg-[#1e293b] hover:bg-[#334155] rounded-full text-white transition-colors cursor-pointer"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
          ...
          <div 
            className="flex-1 w-full max-w-4xl flex items-center justify-center p-4"
          ...
          >
            {is3DModelActive ? (
              ...
              <img
                src={currentItem.url}
                alt={pageTitle}
                className="max-h-[75vh] w-auto object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.9)]"
              />
            )}
          </div>

          <div className="text-center text-sm text-[#94a3b8] font-mono">
            {currentItem.caption || pageTitle}
          </div>
        </div>
      )}
    </div>
